...
async function insertPayment(client, document) {
  const db = client.db();
  const result = await db.collection("price_per_terminal").insertOne(document);
}
...
async function StatusChanging(req, res) {
  if (req.method === "POST") {
    ...
    if (driver_vehicle_type === "vip") {
      ...
      await insertPayment(newPrice);
    }
    if (driver_vehicle_type === "bus") {
      let price = 100;
      let newPrice = {
        pickupLocation: pickupLocation,
        destination: destination,
